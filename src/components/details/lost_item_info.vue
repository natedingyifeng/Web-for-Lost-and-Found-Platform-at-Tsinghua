    ...
    <el-card class='card'>
      <el-form class="form"
          ...
          <el-row>
            ...
            <el-col :span=5>
              <el-form-item label='创建者姓名'>
                <el-input v-model="lost_notice.author.username"
                          :disabled="true"
            ...
            <el-col :span=7>
              ...
              <el-date-picker
                v-model="lost_notice_created_at"
                type="datetime"
                :disabled="true"
            ...
            <el-col :span=7>
              ...
              <el-date-picker
                v-model="lost_notice_updated_at"
                type="datetime"
                :disabled="true"
                format="yyyy-MM-dd HH:mm:ss">
              </el-date-picker>
            ...
            <el-col :span=1>
              <el-form-item>
                <el-button type='primary'
                          @click="enterUser(lost_notice.author.id)"
        ...
        <el-form-item label='丢失物品种类'
          ...
          <el-row>
            <el-col :span=3>
              <el-select v-model="lost_notice.property.template"
                     :disabled=notEdit>
                <el-option v-for="item in lost_notice_item_options"
                          :key="item.name"
                          :label="item.name"
                          :value="item.name">
                </el-option>
        ...
        <el-form-item label='详情描述'
                      class="label">
          <el-input v-model="lost_notice.description"
                    type='textarea'
                    :disabled=notEdit></el-input>
        </el-form-item>
        <el-form-item label='标签'
                      class="label">
          <el-tag v-for="item in lost_notice.property.tags" 
                  :closable=!notEdit
        ...
        <el-form-item label='状态'
                      class="label">
          <el-select v-model="lost_notice.status"
                     :disabled=notEdit>
            <el-option v-for="item in lost_notice_status_options"
                       :key="item.value"
                       :label="item.label"
                       :value="item.value">
            </el-option>
        ...
        <el-timeline width="60%">
          <el-timeline-item
            ...
            placement="top">
            <el-card>
              <el-link target="_blank" @click="enterLostNotice(activity.found_notice)">{{"失物招领: "+activity.found_property_name+"#"+activity.found_notice}}</el-link>
              <p>{{"拾物名称："+activity.found_property_name}}</p>
              <p>{{"拾物描述："+activity.found_notice_description}}</p>
            </el-card>
          </el-timeline-item>
        </el-timeline>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    DeleteFoundNotice() {
      axios.delete('/lost-notices/' + this.id + '/', {
        headers: {
          Authorization: 'Bearer ' + this.$store.getters.getUserAccessToken
        }
      })
    ...
    handleFoundImageRemove(file) {
      ...
      axios({
        ...
        .then((response) => {
          this.lost_notice_images.splice(index, 1);
          this.lost_notice_images_urls.splice(index, 1);
          if(this.lost_notice_images_urls.length < 3)
          {
    ...
    imageNoticeDeleteUpdate(){
      this.lost_notice.images = JSON.parse(JSON.stringify(this.lost_notice_images_urls))
      this.lost_notice_origin.images = JSON.parse(JSON.stringify(this.lost_notice_images_urls))
      axios.put('/lost-notices/'+this.id+'/', this.lost_notice_origin, {
        headers: {
          Authorization: 'Bearer ' + this.$store.getters.getUserAccessToken
        }
      })
    ...
    saveImage(file,fileList) {
      // this.lost_notice_images_urls.push(file.url)
      ...
      axios({
        ...
        .then((response) => {
          // this.lost_notice_images_urls.pop()
          if(this.lost_notice_images_urls.length >= 3)
          {
            this.upload_show = true
          }
          this.lost_notice_images_urls.push({url:response.data.url[0]})
    ...
    createContact() {
      if(this.checkContactValidation() == true)
      {
        ...
        this.lost_notice.contacts.push(tem)
        this.contact_show = JSON.parse(JSON.stringify(this.lost_notice.contacts))
        for(let i=0;i<this.contact_show.length;i++)
        {
          this.contact_show[i].method=this.Contact[this.contact_show[i].method]
    ...
    editContact() {
      if(this.checkContactValidation() == true)
      {
        this.lost_notice.contacts.splice(this.edit_contact_id, 1)
    ...
    handleDelete(index, row) {
      this.lost_notice.contacts.splice(index, 1)
      this.contact_show = JSON.parse(JSON.stringify(this.lost_notice.contacts))
      for(let i=0;i<this.contact_show.length;i++)
      {
        this.contact_show[i].method=this.Contact[this.contact_show[i].method]
    ...
    handleClose(item) {
      this.lost_notice.property.tags.splice(this.lost_notice.property.tags.indexOf(item), 1);
    },
    showInput() {
    ...
    handleInputConfirm() {
      ...
      if (inputValue) {
        this.lost_notice.property.tags.push({name: inputValue})
      }
    ...
    confirm: function () {
      const newData = {}
      for (const key in this.lost_notice) {
        if (this.lost_notice[key] !== '' && key !='') {
          newData[key] = this.lost_notice[key]
        }
      }
      let is_edited = false
      ...
      {
        let data=new FormData();
        data.append('id', this.lost_notice.id)
        data.append('contacts', this.lost_notice.contacts)
        data.append('property', this.lost_notice.property)
        ...
        data.append('return_datetime', this.lost_notice.return_datetime)
        data.append('extra', this.lost_notice.extra)
        data.append('created_at', this.lost_notice.created_at)
        data.append('updated_at', this.lost_notice.updated_at)
        data.append('return_user', this.lost_notice.return_user)
        data.append('images', this.lost_notice.images)
        axios.put('/lost-notices/'+this.id+'/', data, {
          headers: {
            Authorization: 'Bearer ' + this.$store.getters.getUserAccessToken
          }
        })
